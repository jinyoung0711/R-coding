...
#bigdata_korea <- file("키워드_데이터분석_연관어분석용_외국.txt", encoding="UTF-8")
bigdata_korea <- file("키워드_빅데이터_연관어분석용_외국.txt", encoding="UTF-8")
bigdata_korea2 <- readLines(bigdata_korea) # 줄 단위 데이터 생성
# incomplete final line found on - Error 발생 시 UTF-8 인코딩 방식으로 재 저장
close(bigdata_korea) 
# 불용어 제거
txt = readLines('stopwords.txt') 
cnt_txt <- length(txt)
cnt_txt
for( i in 1:cnt_txt) {
  bigdata_korea2 <-gsub((txt[i]),"",bigdata_korea2)      
} 
...
filter1 <- function(x){
  nchar(x) <= 15 && nchar(x) >= 4# && is.english(x)
}
filter2 <- function(x){
  Filter(filter1, x)
}
...
tranrules <- apriori(wordtran, parameter=list(supp=0.01, conf=0.25)) 
tranrules <- apriori(wordtran, parameter=list(supp=0.01, conf=0.8)) 
tranrules <- apriori(wordtran, parameter=list(supp=0.01, conf=0.25)) 
...
ruleg <- graph.edgelist(rulemat[c(2:37),], directed=F) # [1,]~[11,] "{}" 제외
ruleg

#  edgelist 시각화
#X11()
plot.igraph(ruleg, vertex.label=V(ruleg)$name, edge.label.cex=50, edge.arrow.width=10,
