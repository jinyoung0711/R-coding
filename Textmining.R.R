...
install.packages(c("hash", "tau", "Sejong", "RSQLite", "devtools", "bit", 
                   ...
                   "later", "sessioninfo", "xopen","bit64", "blob","DBI","memoise",
                   "plogr","covr","DT","rcmdcheck","rversions"), type="binary")
...
buildDictionary(data.frame(c("R 프로그래밍","페이스북","소셜네트워크"), c("ncn"))) 
# ncn -명사지시코드
...
marketing <- file("키워드_빅데이터_연관어분석용.txt", encoding="UTF-8")
...
# (1) URL 요청
url <- "https://www.chosun.com/nsearch/?query=%EC%98%A4%EC%A7%95%EC%96%B4%EA%B2%8C%EC%9E%84&siteid=&sort=1&date_period=all&date_start=&date_end=&writer=&field=&emd_word=&expt_word=&opt_chk=false&app_check=0&website=www,chosun&category="
web <- GET(url)
web
...
news <- xpathSApply(rootNode, "//div[@class='layout--bg']", xmlValue)
news
