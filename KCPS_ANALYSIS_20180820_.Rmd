...
names(d1)[1] <- c("ID")
```
FID와 IID는 동일한 값이기 때문에 새로운 변수 ID로 변경하였다.
...
d1$ALCO_AMOUNT_B <- ifelse(!is.na(d1$ALCO_AMOUNT_B), d1$ALCO_AMOUNT_B, round(mean(d1$ALCO_AMOUNT_B, na.rm=T))) # 85개의 이상치, 16으로 대체
```
...
```
SBP_B는 수축기혈압(mmHg), DBP_B는 이완기혈압(mmHg)으로 고혈압 진단시 사용되는 척도이다.
일반적으로 정상 혈압은 120/80(수축기/이완기)mmHg 이하로, 고혈압은 140/90mmHg 이상, 저혈압은 90/60mmHg 이하이다.
새로운 변수 HBP_B, LBP_B, BP_B를 생성하였다. 
HBP_B : 고혈압 여부 ( 0: 정상, 1: 고혈압), LBP_B : 저혈압 여부 ( 0: 정상, 1: 고혈압), BP_B : 혈압 이상여부 ( 0: 정상, 1: 혈압 이상치)
이후 SBP_B, DBP_B 변수는 이후 제거하였다.

* ##### CHO_B, LDL_B, TG_B, HDL_B, FBS_B, GOT_B, GPT_B, GGT_B, URIC_B 변수 전처리

```{r}
# CHO_B, LDL_B, TG_B, HDL_B, FBS_B, GOT_B, GPT_B, GGT_B, URIC_B 이상치 처리
...
```
CHO_B : 총콜레스테롤(mg/ dl), LDL_B : 저밀도콜레스테롤(mg/ dl), TG_B : 중성지방(mg/ dl), HDL_B : 고밀도콜레스테롤(mg/ dl), FBS_B : 공복혈당(mg/ dl), GOT_B : AST(IU/L), GPT_B : ALT(IU/L), GGT_B : r-GTP(IU/L), URIC_B : 요산 (mg/ dl)
...
암의 과거력들은 새로운 변수 PAR(과거력들의 합)으로 나타내고, PCAN80 ~ PCAN89는 제거하였다.
...
암의 가족력들은 새로운 변수 FAM(가족력들의 합)으로 나타내고, FCAN80 ~ FCAN89는 제거하였다.
...
```{r, echo=FALSE}
d1_SMOKA <- d1[, 20:27]
d1_SMOKA$SMOKA_AMOUNT <- d1$SMOKA_MOD_B
d1_SMOKA_cor <- cor(d1_SMOKA)
corrplot(d1_SMOKA_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")
```
흡연량은 폐암과 높은 상관관계를 보였고, 직장암 같은 경우 상관관계가 존재하지 않는다.

* ##### 음주량과 암 상관관계 분석
```{r, echo=FALSE}
d1_ALCO <- d1[, 20:27]
d1_ALCO$ALCO_AMOUNT <- d1$ALCO_AMOUNT_B
d1_ALCO_cor <- cor(d1_ALCO)
corrplot(d1_ALCO_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")

```

음주량의 경우 간암과 가장 높은 상관관계를 보이고 있으며, 다른 암들과는 근소한 상관관계를 보인다.

* ##### 운동여부와 암 상관관계 분석
```{r, echo=FALSE}
d1_EXER <- d1[, 20:27]
d1_EXER$EXER <- d1$EXER_B
d1_EXER_cor <- cor(d1_EXER)
corrplot(d1_EXER_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")
```

운동여부는 유방암과 가장 높은 상관관계를 보이고 있다.

* ##### 건강지수와 암 상관관계 분석
```{r, echo=FALSE}
d1_HEALTH <- d1[, 20:27]
d1_HEALTH$HEALTH <- d1$HEALTH_B
d1_HEALTH_cor <- cor(d1_HEALTH)
corrplot(d1_HEALTH_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")
```

BMI와 허리둘레로 판단한 건강지수와 암의 상관관계를 파악하였다.
갑상선암과 유방암에서 양의 상관관계를 보였으며, 다른 암과는 근소한 상관관계를 보인다.

* ##### 성별과 암 상관관계 분석
```{r, echo=FALSE}
d1_SEX <- d1[, 20:27]
d1_SEX$SEX <- d1$SEX1
d1_SEX_cor <- cor(d1_SEX)
corrplot(d1_SEX_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")
```

성별은 갑상선암과 유방암에서 높은 상관관계를 보이고 있다.
...
d1_BP$BP <- d1$BP_B
d1_BP_cor <- cor(d1_BP)
corrplot(d1_BP_cor, method="shade", addshade="all", shade.col=NA, tl.col="red", 
tl.srt=40, diag=FALSE, addCoef.col="black")
```

혈압은 간암과 높은 상관관계를 보이고 있다.